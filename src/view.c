...
//g_print("key-press-event: 0x%X\n", event->keyval);
	...
	case GDK_Tab:
		...
		case GDK_KEY_F11:
			//windowgdk = gtk_widget_get_window(view);
			//state = gdk_window_get_state(GDK_WINDOW(windowgdk));
			//g_print("%d", state);
			windowgtk = GTK_WINDOW(gtk_widget_get_toplevel(view));
			//windowgtk = GTK_WINDOW(pub->mw->window);

			//if (state & GDK_WINDOW_STATE_FULLSCREEN) {
			if (windowf){
				windowf = FALSE;
				gtk_window_unfullscreen(GTK_WINDOW(windowgtk));
			}
			else {
				windowf = TRUE;
				gtk_window_fullscreen(GTK_WINDOW(windowgtk));
			}
			break;
/*		
	case GDK_F11:
		// window = GTK_WINDOW(gtk_widget_get_toplevel(view));
		window = GTK_WINDOW(gtk_widget_get_ancestor (view, GTK_TYPE_WINDOW));
		//if (gtk_window_is_maximized (window) == TRUE) {
		// if ((event->state & GDK_WINDOW_STATE_FULLSCREEN) != 0) {
		// is_full_screen = gdk_window_get_state (gtk_widget_get_window (view)) & GDK_WINDOW_STATE_FULLSCREEN;
		windowf = gdk_window_get_state (gtk_widget_get_window (view)) & GDK_WINDOW_STATE_FULLSCREEN;
		is_full_screen = gdk_window_get_state (gtk_widget_get_window (view));
		
		g_print("%d",is_full_screen);
		
		if (is_full_screen) {
			gtk_window_unfullscreen(window);
			g_print("undoing fullscreen");
			//gtk_window_unmaximize(window);
		}
		else { 
			gtk_window_fullscreen(window);
			g_print("going fullscreen");
...
/* Change default font and color throughout the widget */
	...
	gtk_css_provider_load_from_data (provider, "textview text { \
	   background-color: #dedede;  }", -1, NULL);
